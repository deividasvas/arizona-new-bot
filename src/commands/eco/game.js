...
const AnswersGame = require('../../models/QuestionsGame');
const {
  coinsRates: {
    betMin,
    betMax,
  ...
  combinationsRockPaperScissors, questionsWords
...
const words = async ({ bot, interaction, author, profile, bet, guild, channel }) => {
  ...
  const questionsWords = await AnswersGame.find();
  ...
  if (!messages.size) {
    await setUserCoinsParam(author.id, guild.id, 'coins', ({ coins }) => coins - bet)
    return interaction.followUp({
      embeds: [
        new EmbedBuilder()
          .setTitle(`💰 | Игра в слова!`)
          .setColor(Colors.Blue)
          .setDescription(`**Вы не успели ответить на вопрос!\nС Вашего счёта была снята сумма на которую должна была быть игра!\nИгра окончена.**`)
          .setAuthor({
            name: guild.name, iconURL: guild.iconURL()
          })
          .setFooter({
...
const rockPaperScissors = async ({ bot, interaction, author, profile, bet, guild, channel }) => {
  ...
  const messages = await channel.awaitMessages({
    filter: (msg) => {
      ...
      ) && combinationsRockPaperScissors.map(combination => combination.item).includes(msg.content.toLowerCase())
    },
    time: convertMinutesToMs(1),
    max: 1
  })
