...
module.exports = {
  ...
  run: async ({ bot, interaction, author, guild, args, channel }) => {
    ...
    const roleInWhiteList = userForKick.roles.cache.find((role) =>
      whiteListRolesForPunish.includes(role.id)
      ...
      return interaction.reply({
        ...
        embeds: [
          new EmbedBuilder()
            .setTitle(`❌ | Ошибка!`)
            .setDescription(
              `**Пользователя ${userForKick} невозможно наказать потому, что, у человека есть роль <@&${roleInWhiteList.id}> которая находится в белом списке.**`
            )
            .setColor(`Red`)
            .setAuthor({
              name: guild.name,
