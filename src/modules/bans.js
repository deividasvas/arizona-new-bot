...
module.exports = {
    ...
    */
    ...
    async banUser(bot, interaction, userId, days, reason, moderatorId) {
        ...
        // Все айди каналов сервера.
        const channelsId = getGuildChannelsId(interaction.guild.id)
        ...
        await log(4, {
            guildId: interaction.guild.id, // ID сервера
            ...
            moderatorId: user.id, // ID автора сообщения
            moderatorTag: user.user.tag, // Tag автора сообщения
            moderatorNick: user.displayName, // Серверный ник автора сообщения
            reason,
        })
    ...
    async run({bot, interaction, member: user, guild}) {
        ...
        if (
            !user.roles.cache.find((role) => getAllRolesIdModers(rolesId).includes(role.id))
        ) {
