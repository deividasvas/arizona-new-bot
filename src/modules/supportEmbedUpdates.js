  ...
  */
  ...
  async update () {
    ...
    for (const [id, guild] of bot.guilds.cache) {
      // Все айди каналов сервера в объекте.
      const channelsId = getGuildChannelsId(id);
      const supportChannel = guild.channels.cache.get(channelsId.support)

      const messagesOfBot = []

      for (const [id, message] of Array.from((
        await supportChannel.messages.fetch()
      ).filter(message => message.author.id === bot.user.id)).reverse())
      {
