...
const logInDataBase = require('../components/log');
    ...
    */
    ...
    async addRolesRequest({interaction, bot, member, rolesId, guild, channelsId}) {
        if (createRequestForRole.has(member.id)) {
            // Количество секунд через сколько можно будет сделать новый запрос.
    ...
    async requestGiveRole({interaction, guild, rolesId, member, channelsId, bot}) {
        ...
        logInDataBase(30, {
            ...
            discordId: userForGiveRole.id, // ID упомянутого участника
            discordTag: userForGiveRole.user.tag, // Tag упомянутого участника
            discordNick: userForGiveRole.displayName, // Серверный ник упомянутого участника
            ...
            roleId: roleId,
            roleName: guild.roles.cache.get(roleId).name,
            value: roleId,
        })
    ...
    async requestDenyRole({bot, interaction, guild, member, channelsId, rolesId}) {
        ...
        logInDataBase(31, {
            guildId: guild.id, // ID сервера
            discordId: userForDenyRole.id, // ID упомянутого участника
            discordTag: userForDenyRole.user.tag, // Tag упомянутого участника
            discordNick: userForDenyRole.displayName, // Серверный ник упомянутого участника
            moderatorId: member.id, // ID автора сообщения
            moderatorTag: member.user.tag, // Tag автора сообщения
            moderatorNick: member.displayName, // Серверный ник автора сообщения
            roleId: roleId,
            ...
            embeds: [
                new EmbedBuilder()
                    ...
                    .addFields([
                        ...oldEmbed.fields, {
                            name: `Важно!`,
                            value: `У игрока была запрошена статистика модератором \`${member.displayName}\``
                        }
                    ])
            ]
        })
